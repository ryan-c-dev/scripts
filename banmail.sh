...
if [ -n "$1" ]; then
	...
	echo "$choice" | grep -i "^y$" &&
		...
		eval "ssh intfur 'echo blacklist_from $domain >> /etc/spamassassin/local.cf;systemctl reload spamassassin &'"
else
	...
	echo "$choice" | grep -i "^y$" &&
		...
		eval "ssh intfur 'echo blacklist_from $mail >> /etc/spamassassin/local.cf;systemctl reload spamassassin &'"
		eval "ssh intfur 'service spamassassin restart"

fi

ssh rc 'service spamassassin restart'
ssh intfur 'service spamassassin restart'
